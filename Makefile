...
DEPENDENCIES := $(ENV)/.pipenv-$(shell checksum Pipfile*)
METADATA := *.egg-info
...
.PHONY: flake8
flake8: install
	$(FLAKE8) $(PACKAGES) $(CONFIG) --config=config/flake8

.PHONY: pycodestyle
pycodestyle: install
	$(PYCODESTYLE) $(PACKAGES) $(CONFIG) --config=config/pycodestyle.ini

.PHONY: pydocstyle
pydocstyle: install
	$(PYDOCSTYLE) $(PACKAGES) $(CONFIG) --config=config/pydocstyle.ini

# TESTS #######################################################################

PYTEST := pipenv run pytest
COVERAGE := pipenv run coverage --rcfile=config/coveragerc
COVERAGE_SPACE := pipenv run coverage.space --rcfile=config/coveragerc
