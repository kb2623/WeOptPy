...
class EvolutionStrategyMpL(EvolutionStrategy1p1):
	...
	def set_parameters(self, lam=45, **ukwargs):
		...
		See Also:
			* :func:`WeOptPy.algorithms.EvolutionStrategy1p1.setParameters`
		"""
		EvolutionStrategy1p1.set_parameters(self, init_pop_func=default_individual_init, **ukwargs)
		self.lam = lam
	...
	def run_iteration(self, task, c, fpop, xb, fxb, ki, *args, **dparams):
		...
		cn = objects2array([IndividualES(x=self.mutate_rand(c, task), task=task, rnd=self.Rand) for _ in range(self.lam)])
		cn = np.append(cn, c)
